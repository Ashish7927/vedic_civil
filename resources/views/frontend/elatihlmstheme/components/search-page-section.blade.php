            ...
            <div class="row">
                <div class="col-lg-4 col-xl-3">
                    <x-class-page-section-sidebar :level="$level" :type="$type" :categories="$categories"
                                                  :category="$category" :languages="$languages" :language="$language" :mode="$mode" :rate="$rate" :duration="$duration" :content-provider="$content_provider" :start-price="$startprice" :end-price="$endprice"
                                                  :startDuration="$startDuration" :endDuration="$endDuration":cpdata="$cpdata" :version="$version"/>
                </div>
                            ...
                            <div class="box_header d-flex flex-wrap align-items-center justify-content-between">
                                ...
                                <form onsubmit="{{route('search')}}">
                                        <div class="input-group theme_search_field mb_30">
                                            ...
                                            <input type="text" class="form-control search" name="query" placeholder="Search for courses, skills and videos" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Search for courses'" value="<?php if(isset($_GET['query'])){ echo $_GET['query']; } ?>" >
                                    ...
                                    <div class="short_select d-flex align-items-center">
                                        ...
                                        <select class="small_select" id="order">
                                           <option data-display="None">{{__('frontend.None')}}</option>
                                            <option
                                                value="popularity" {{$order=="popularity"?'selected':''}}>{{__('frontend.Popularity')}}</option>
                                            <!--<option
                                                value="price" {{$order=="price"?'selected':''}}>{{__('frontend.Price')}}</option>-->
                                            <option
                                                value="date" {{$order=="date"?'selected':''}}>{{__('frontend.Date')}}</option>
                                                <option
                                                value="alphabet" {{$order=="alphabet"?'selected':''}}>{{__('frontend.Alphabet')}}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <h1>
                                        <div class="thumb">
                                            <img style="width: 50px"
                                                 src="{{ asset('public/frontend/elatihlmstheme') }}/img/not-found.png"
                                                 alt="">
                                            @if(!isset($search))
