                            ...
                            @foreach($logs as $log)
                                <tr>
                                    <?php
                                        ...
                                        $trans_id = "";
                                        if ($tracking != 1) {
                                            $checkout = \Modules\Payment\Entities\Checkout::where("tracking", $tracking)->first();
                                            if ($checkout->response) {
                                                $resp = json_decode($checkout->response);
                                                if ($resp->TransId)
                                    ...
                                    <td>{{ getPriceFormat(@$log->myll_revenue / $tax) }}</td>
                                    <td>
                                        @if ($log->discount_amount != 0)
                                            {{ getPriceFormat(@$log->discount_amount) }}
                                        @endif
                                    </td>
                                    <td>{{ showDate(@$log->created_at ?? now()) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
