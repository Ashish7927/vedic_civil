...
class CourseDeatilsPageSection extends Component
{
    ...
    {
        $related = Course::where('category_id', $this->course->category_id)->with('activeReviews', 'enrollUsers', 'cartUsers', 'lessons')->where('id', '!=', $this->course->id)->take(2)->get();

        $auther_courses = Course::with('lessons')->where('id', '!=', $this->course->id)->where('user_id', $this->course->user_id)->where('status', 1)->take(2)->get();
        ...
        return view(theme('components.course-details-page-section'), compact('is_cart', 'levels', 'related', 'userRating', 'lessons', 'total', 'isFree', 'isBookmarked', 'course_exercises', 'reviewer_user_ids', 'course_enrolled_std', 'auther_courses'));
    }
}
