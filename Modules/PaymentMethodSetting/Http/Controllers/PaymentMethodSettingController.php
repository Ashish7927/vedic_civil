    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('logo')) {
                ...
                $upload_path = 'uploads/gateway/';
                $img->save($upload_path . $name);
                $method->logo = 'uploads/gateway/' . $name;
            }
            $method->save();
...
//                    $value2 = str_replace('/', '\\', $fileName);
            ...
            elseif ($method->method == 'Ipay88') {
                $key1 = 'IPAY_MERCHANT_KEY';
                $key2 = 'IPAY_MERCHANT_CODE';


                $value1 = trim($request->ipay_merchant_key);
                $value2 = trim($request->ipay_merchant_code);
