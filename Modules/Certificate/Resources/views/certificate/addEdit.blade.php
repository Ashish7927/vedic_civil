            ...
            <div class="white-box p-0">
                <div class="add-visitor certificate">
                        ...
                        <div class="col-lg-12">
                            <div class="checkbox_wrap d-flex align-items-center mt-40">
                              <label for="is_free" class="switch_toggle  mr-2">
                                  <input type="checkbox" class="is_free" id="is_free" name="is_free"
                                  value="1" {{(old('is_free') || (isset( $certificate ) && $certificate->is_free) == '1' ) == '1'?'checked':''}}>
                                    <i class="slider round"></i>
                                </label>
                                <label class="mb-0">This certificate is for free course</label>
                            </div>
                        </div>
                    </div>
                    <div class="row mt-25 padding_40">
                    ...
                    <div class="row mt-25 padding_40">
                        <div class="col-md-12">
                            <label class="primary_input_label"
                                   for=""> Content Provider's  Name</label>
                        </div>
                        <div class="col-md-6">
                            <input onclick="getNew()" type="radio" checked
                                   class="common-checkbox cp_name" name="cp_name" id="yes0"
                                   value="1" {{isset($certificate) && $certificate->cp_name==1?"checked":""}}>
                            <label for="yes0">{{__('certificate.Yes')}}</label>
                        </div>
                        <div class="col-md-6">
                            <input onclick="getNew()" type="radio"
                                   class="common-checkbox cp_name" name="cp_name" id="no0"
                                   value="0" {{isset($certificate) && $certificate->cp_name!=1?"checked":""}}>
                            <label for="no0">{{__('certificate.No')}}</label>
                        </div>
                    </div>
                    ...
                    <div class="row mt-25 padding_40">
                        <div class="col-lg-6">
                            <div class="input-effect">
                                <label> {{__('certificate.Position X')}}  </label>
                                <input type="number"
                                       {{ $errors->has('cp_position_x') ? ' autofocus' : '' }}
                                       placeholder="{{__('certificate.Position X')}}"
                                       class="primary_input_field change-input date_position_x {{ $errors->has('cp_position_x') ? ' is-invalid' : '' }}"
                                       name="cp_position_x" id="cp_position_x"
                                       value="{{isset($certificate)? $certificate->cp_position_x:(old('cp_position_x')!=''?(old('cp_position_x')):0)}}">
                                <span class="focus-border textarea"></span>
                                @if ($errors->has('cp_position_x'))
                                    <span
                                        class="error text-danger"><strong>{{ $errors->first('cp_position_x') }}</strong></span>
                                @endif
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="input-effect">
                                <label> {{__('certificate.Position Y')}} </label>
                                <input type="number"
                                       {{ $errors->has('cp_position_y') ? ' autofocus' : '' }}
                                       placeholder="{{__('certificate.Position Y')}}"
                                       class="primary_input_field  change-input  cp_position_y {{ $errors->has('cp_position_y') ? ' is-invalid' : '' }}"
                                       name="cp_position_y" id="cp_position_y"
                                       value="{{isset($certificate)? $certificate->cp_position_y:(old('cp_position_y')!=''?(old('cp_position_y')):0)}}">
                                <span class="focus-border textarea"></span>
                                @if ($errors->has('cp_position_y'))
                                    <span
                                        class="error text-danger"><strong>{{ $errors->first('cp_position_y') }}</strong></span>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row mt-25 padding_40">
                        <div class="col-md-12">
                            <div class="input-effect">
                                <label> {{__('certificate.Font Family')}} </label>
                                <select class="primary_select cp_font_family" onchange="getNew()"
                                        id="cp_font_family"
                                        name="cp_font_family">
                                    @foreach($font_list as $key=>$value)
                                        <option
                                            value="{{$key}}" {{isset($certificate)?$certificate->cp_font_family==$key?'selected':'':''}}>
                                            {{$value}}
                                        </option>
                                    @endforeach
                                </select>
                                @if ($errors->has('cp_font_family'))
                                    <span
                                        class="error text-danger"><strong>{{ $errors->first('cp_font_family') }}</strong></span>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row mt-25 padding_40">
                        <div class="col-lg-6">
                            <div class="input-effect">
                                ...
                                <input type="number" required
                                       placeholder="{{__('certificate.Font Size')}}"
                                       class="primary_input_field cp_font_size change-input {{ $errors->has('cp_font_size') ? ' is-invalid' : '' }}"
                                       name="cp_font_size" id="cp_font_size"
                                       {{ $errors->has('cp_font_size') ? ' autofocus' : '' }}
                                       value="{{isset($certificate)? $certificate->cp_font_size:(old('cp_font_size')!=''?(old('cp_font_size')):50)}}">
                                <span class="focus-border textarea"></span>
                                @if ($errors->has('cp_font_size'))
                                    <span
                                        class="error text-danger"><strong>{{ $errors->first('cp_font_size') }}</strong></span>
                                @endif
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="input-effect">
                                <label> {{__('certificate.Font Color')}} </label>
                                <input type="color" required
                                       class="primary_input_field change-input cp_font_color  {{ $errors->has('cp_font_color') ? ' is-invalid' : '' }}"
                                       name="cp_font_color" id="cp_font_color"
                                       {{ $errors->has('font_color') ? ' autofocus' : '' }}
                                       value="{{isset($certificate)? $certificate->cp_font_color:(old('cp_font_color')!=''?(old('cp_font_color')):'#383CC1')}}">
                                <span class="focus-border textarea"></span>
                                @if ($errors->has('cp_font_color'))
                                    <span
                                        class="error text-danger"><strong>{{ $errors->first('cp_font_color') }}</strong></span>
                                @endif
                            </div>
                ...
                {{-- Start Certificate Number Section --}}
                    ...
                    <div class="row mt-25 padding_40">
                        <div class="col-md-12">
                            <label class="primary_input_label"
                                   for=""> {{__('certificate.Is Partner')}}</label>
                        </div>
                        <div class="col-md-6">
                            <input onclick="getNew()" type="radio"
                                   class="common-checkbox is_partner" name="is_partner" id="partner_yes"
                                   value="1" {{isset($certificate) && $certificate->is_partner==1?"checked":""}}>
                            <label for="partner_yes">{{__('certificate.Yes')}}</label>
                        </div>
                        <div class="col-md-6">
                            <input onclick="getNew()" type="radio"
                                   class="common-checkbox is_partner" name="is_partner" id="partner_no"
                                   value="0" {{isset($certificate) && $certificate->is_partner!=1?"checked":""}}>
                            <label for="partner_no">{{__('certificate.No')}}</label>
                        </div>
                    </div>
                </div>
