...
class QuizController extends Controller
{
    ...
    public function quizSubmit(Request $request)
    {
        ...
        try {
            ...
            if(in_array('F', $request->type)){
                if(isset($allAns) && count($allAns) > 0){
                    foreach($allAns as $key => $value){
                        $ans_id = (int)$value;
                        $option_data = QuestionBankMuOption::find((int)$value);
                        if($option_data){
                            if(isset($option_data->question->type) && $option_data->question->type == 'F'){
                                $question_test_details = QuizTestDetails::where('quiz_test_id', $request->quiz_test_id)->where('qus_id', $key)->first();
                                if(!$question_test_details)
                                {
                                    $question_test_details = new QuizTestDetails();
                                    $question_test_details->quiz_test_id = $request->quiz_test_id;
                                    $question_test_details->qus_id = $key;
                                    $question_test_details->save();
                                }
                                $question_test_details->ans_id = $ans_id;
                                if($option_data->status == 1) //correct
                                {
                                    $question_test_details->status = 1; //true
                                    $question_test_details->mark = $option_data->question->marks;
                                }
                                else{
                                    $question_test_details->status = 0; //false
                                    $question_test_details->mark = 0;
                                }
                                $question_test_details->save();
                                $calculate_marks += $question_test_details->mark;
                            }
                        }
                    }
                    $quiz_marking->marked_by = 0;
                    $quiz_marking->marking_status = 1;
                    $quiz_marking->marks = $calculate_marks;
                    $quiz_test->publish = 1;
                }
            }
            if(in_array('Mt', $request->type)){
                if(isset($allAns) && count($allAns) > 0){
                    $give_marks = true;
                    $marks_per_ques = 0;
                    foreach($allAns as $key => $value){
                        $ans_id = (int)$value;
                        $option_data = QuestionBankMuOption::find($key);

                        if($option_data){
                            if(isset($option_data->question->type) && $option_data->question->type == 'Mt'){
                                /* Split marks for each question */
                                $total_marks = $option_data->question->marks;
                                $total_questins = count($option_data->question->questionMu->where('match_type', 1));
                                    if($total_questins > 0){
                                        if($total_questins == 1){
                                            $marks_per_ques = $total_marks;
                                        }
                                        else{
                                            $marks_per_ques = $total_marks/$total_questins;
                                        }
                                    }
                                /* Split marks for each question */
                                $question_test_details = QuizTestDetails::where('quiz_test_id', $request->quiz_test_id)->where('qus_id', $key)->first();
                                if(!$question_test_details)
                                {
                                    $question_test_details = new QuizTestDetails();
                                    $question_test_details->quiz_test_id = $request->quiz_test_id;
                                    $question_test_details->qus_id = $option_data->question_bank_id;
                                    $question_test_details->match_qus_id = $key;
                                    $question_test_details->save();
                                }
                                $question_test_details->ans_id = $ans_id;
                                if($option_data->match_ans_id == $ans_id) //correct
                                {
                                    $question_test_details->status = 1; //true
                                    $question_test_details->mark = $marks_per_ques;
                                }
                                else{
                                    $question_test_details->status = 0; //false
                                    $question_test_details->mark = 0;
    ...
    public function singleQuizSubmit(Request $request)
    {
        try {
            ...
            elseif($type == "Mt"){
                ...
                $options_data = QuestionBankMuOption::where('question_bank_id', $assign->question_bank_id)->first();
                $totalCorrectAns = count($options_data->question->questionMu->where('match_type', 1));
                ...
                if (!empty($answer)) {
                    foreach ($answer as $key => $ans) {
                        $setAns = new QuizTestDetailsAnswer();
                        $option = QuestionBankMuOption::with('question')->find($key);
                        if ($option) {
                            $setAns->quiz_test_details_id = $quizDetails->id;
                            $setAns->ans_id = $ans;
                            if($option->match_ans_id = (int)$ans){
                                $setAns->status = $option->status;
                            }
                            $setAns->save();
