...
class WebsiteController extends Controller
{
    ...
    {
        try {
            $cities = DB::table('spn_cities')->select('id', 'name')->where('name', 'like', '%' . $request->search . '%')->where('country_id', '=', $request->id)->paginate(10000);
            // dd($cities->toArray());
            $response = [];
            $arr_data = [];
            foreach ($cities->toArray() as $key => $item) {
                if ($key == 'data') {
                    $arr_data = $item;
                }
            }
    ...
    /*History Courses Certificate Download Start*/
    public function historycertificateDownload($id, Request $request)
    {
        try {

            $certificate_record = UserCourseOld::where('id', $id)->first();
            $certificate = Certificate::where('for_course', 1)->first();
            if (!$certificate) {
                Toastr::error(trans('certificate.Right Now You Cannot Download The Certificate'));
                return back();
            ...
            $request->cert_created_at = $certificate_record->EndDate;
            $request->name = $certificate_record->UserName;
            $request->title = $certificate_record->Title;
            $request->course = Course::where('title', $certificate_record->Title)->first();
            $request->user = User::where('email', $certificate_record->Email)->first();
            $certificate = $downloadFile->historymakeCertificate($certificate->id, $request)['image'] ?? '';
            $certificate->encode('png');
            $type = 'png';
            $certificate = 'data:image/' . $type . ';base64,' . base64_encode($certificate);
    ...
    /*History Courses Certificate Download End*/
    public function certificateDownload($certificate_number, Request $request)
    {
        try {
            ...
            //return view('myPDF',compact('certificate'));
            $pdf = PDF::loadView('myPDF', compact('certificate'));
            return $pdf->download($certificate_number . '.pdf');
        } catch (\Exception $e) {
            GettingError($e->getMessage(), url()->current(), request()->ip(), request()->userAgent());
        }
    }
        ...
        try {
            ...
            if (isset($request->quiz_result_id)) {
                ...
                foreach ($all as $key => $i) {
                    ...
                    $quizTestDetails = QuizTestDetails::find($i->id);
                    ...
                    if ($request->quiz_result_id == $i->id) {
                        ...
                        $result['quiz_feedback'] = $onlineQuiz->quiz_feedback;
                        $result['enable_quiz_feedback'] = $onlineQuiz->enable_quiz_feedback;
                        $i->save();
                    } else {
                    ...
                    if ($check && $i->pass == 1) {
                        ...
                        enrolled_course_add_completion_date($lesson, Auth::id(), $i->course_id);
                    }
                }
            }

            $course = Course::withoutGlobalScope('withoutsubscription')->findOrFail($course_id);
            $lesson = Lesson::where('id', $lesson_id)->first();
            $resumption = $lesson->lessonQuiz->is_enabled;
            ...
            $isAdmin = false;
            if (Auth::check()) {
                if (Auth::user()->role_id == 1 || Auth::user()->role_id == 5 || Auth::user()->role_id == 6 || Auth::user()->role_id == isCourseReviewer()) {
                    $isAdmin = true;
                }
            }
            $lesson_ids = [];
            ...
            return view(theme('pages.fullscreen_video'), compact('quizPass', 'alreadyJoin', 'lesson_ids', 'result', 'preResult', 'quizSetup', 'chapters', 'reviewer_user_ids', 'percentage', 'isEnrolled', 'total', 'certificate', 'course', 'lesson', 'lessons', 'resumption'));
        } catch (\Exception $e) {
            GettingError($e->getMessage(), url()->current(), request()->ip(), request()->userAgent());
        }
    }
    ...
    public function enrolfree($id)
    {
        try {
            ...
            if (Auth::check() && ($user->role_id != 1)) {
                ...
                $lesson = LessonComplete::where('course_id', $course->id)->where('user_id', $user->id)->has('lesson')->orderBy('updated_at', 'desc')->delete();

                if (UserEmailNotificationSetup('Course_Enroll_Payment', $enroll->user)) {
                    send_email($enroll->user, 'Course_Enroll_Payment', [
                        'time' => \Illuminate\Support\Carbon::now()->format('d-M-Y ,s:i A'),
                        'course' => $course->title,
                        'currency' => $enroll->user->currency->symbol ?? '$',
                        'price' => ($enroll->purchase_price),
                ...
                if (UserBrowserNotificationSetup('Course_Enroll_Payment', $enroll->user)) {
                ...
                Toastr::success('Successfully Enroled', 'Success');
                return redirect()->back();
            }
        } catch (\Exception $e) {
            GettingError($e->getMessage(), url()->current(), request()->ip(), request()->userAgent());
            ...
            if (Auth::check() && ($user->role_id != 1)) {
                ...
                if (isset($exist)) {
                    ...
                    if (isset($oldCart)) {
                        ...
                        if ($course->discount_price != null && @strtotime($discount_start_date) <= @strtotime($current_date) && @strtotime($discount_end_date) >= @strtotime($current_date)) {
    ...
    public function category()
    {
        ...
        $category = Category::where('status', 1)->orderBy('name', 'asc')
            ->get();
        return view(theme('pages.category'), compact('category'));
    }
    ...
    public function contactMsgSubmit(Request $request)
    {
        ...
        if (config('captcha.for_contact')) {
            if (empty($_POST['h-captcha-response'])) {
                Toastr::error('H-Captcha Validation failed', trans('common.Failed'));
                return redirect()->back();
            }
            $validate_rules = [
                'name' => 'required',
        ...
        try {
            // Validate captcha choosen
            $data = array(
                'secret' => "ES_2c9ff651c5574d2489cbcacb5e0e43eb",
            ...
            if ($responseData->success) {
                ...
                Toastr::error('H-Captcha Valication Failed', trans('common.Failed'));
                return redirect()->back()->with(['input']);
            }
        } catch (Exception $e) {
            Log::error('Error in H-captcha response verification of contact request' . $e->getMessage());
        }
    }
            ...
            if (!empty($items)) {
                foreach ($items as $key => $cart) {
                    ...
                    if ($check) {
                        ...
                        $price_detail        = $cart->course->price;
                        $discount_detail     = $cart->course->discount_price;
                        $discount_start_date = $cart->course->discount_start_date;
                        $discount_end_date   = $cart->course->discount_end_date;
                        $current_date        = date('Y-m-d');

                        if ($cart->course->discount_price != null && @strtotime($discount_start_date) <= @strtotime($current_date) && @strtotime($discount_end_date) >= @strtotime($current_date)) {
    ...
    public function lessonComplete(Request $request)
    {
        ...
        try {
            ...
            enrolled_course_add_completion_date($lesson, Auth::id(), $request->course_id);
    ...
    public function continueCourse($slug)
    {
        try {
            ...
            $user = Auth::user();
            $course = Course::withoutGlobalScope('withoutsubscription')->where('slug', $slug)->first();
            if (!$course) {
                Toastr::error(trans('common.Operation failed'), trans('common.Failed'));
                return redirect()->route('courseDetailsView', $slug);
            }
    ...
    public function updateenrolledcource()
    {
        $course = Course::all();
        foreach ($course as $key) {
            $totalenrolled = $key->total_enrolled;
            // $enrolled = DB::table('course_enrolleds')->where('course_id', $key->id)->count();
            $enrolled = DB::table('course_enrolleds')->where('course_id', $key->id)->groupBy('user_id')->get();
            $updatecourse = Course::find($key->id);
            // $updatecourse->total_enrolled = $enrolled;
            $updatecourse->total_enrolled = $enrolled->count();
            $updatecourse->save();
        }
        echo 'success';
    }

    public function interval_auth_check()
    {
    ...
    public function state_data_with_ajax(Request $request)
    {
        if ($request->ajax()) {

            $term = trim($request->term);
            $country = trim($request->country);
            $results = state_data_with_select2_search_ajax($term, $country, 10);

            return $results;
        }
    }

    public function cp_data_with_ajax(Request $request)
    {

        if ($request->ajax()) {
            $term = trim($request->term);
            $results = cp_data_with_select2_search_ajax($term, 10);

            return $results;
        }
    }

    public function cpTermsAndCondition()
    {
        return view(theme('pages.cp_terms_and_condition'));
    }

    public function cp_which_not_added_with_ajax(Request $request)
    {
        if ($request->ajax()) {

            $term = trim($request->term);

            $results = cp_data_which_not_added_with_select2_search_ajax($term, 10, $request); //term, pagination

            return $results;
        }
    }

    public function corporate_which_not_added_with_ajax(Request $request)
    {
        if ($request->ajax()) {

            $term = trim($request->term);

            $results = corporate_data_which_not_added_with_select2_search_ajax($term, 10, $request); //term, pagination

            return $results;
        }
    }

    public function packages_which_not_added_with_ajax(Request $request)
    {
        if ($request->ajax()) {

            $term = trim($request->term);

            $results = packages_data_which_not_added_with_select2_search_ajax($term, 10, $request); //term, pagination
