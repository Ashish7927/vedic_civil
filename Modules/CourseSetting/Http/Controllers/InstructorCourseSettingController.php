...
class InstructorCourseSettingController extends Controller
{
    ...
    public function saveChapter(Request $request)
    {
        ...
        if ($request->input_type == 1) {
            try {
                $loginUser = Auth::user();
                if ($loginUser->role_id == 1 || $loginUser->role_id == 5 || $loginUser->role_id == 6) {
                ...
                //dd($loginUser->role_id == 1 || $loginUser->role_id == 5 || $loginUser->role_id == 6);
                if (isset($course)) {
    ...
    public function deleteChapter($chapter, $course_id)
    {
        ...
        try {
            $user = Auth::user();
            if ($user->role_id == 1 || $user->role_id == 5 || $user->role_id == 6) {
                $course = Course::withoutGlobalScope('withoutsubscription')->where('id', $course_id)->first();
            } else {
                $course = Course::withoutGlobalScope('withoutsubscription')->where('id', $course_id)->where('user_id', Auth::id())->first();
            }
        ...
        if ($request->input_type == 1) {
            try {
                ...
                if ($user->role_id == 1 || $user->role_id == 5 || $user->role_id == 6) {
                    ...
                    $course = Course::withoutGlobalScope('withoutsubscription')->where('id', $request->course_id)->where('user_id', $user->id)->first();
                }
            ...
            try {
                ...
                if (isset($course) && isset($chapter)) {
                    ...
                    if (isModuleActive('Org') && $request->fileType != 2) {
                        ...
                        if ($request->get('host') == "Vimeo") {
                            if (config('vimeo.connections.main.upload_type') == "Direct") {
                                vimeo_upload_video_in_folder($request);
                                // $courseSettingController = new CourseSettingController();
                                // $lesson->video_url = $courseSettingController->uploadFileIntoVimeo($request->name, $request->vimeo);
                            } else {
                                $lesson->video_url = $request->vimeo;
