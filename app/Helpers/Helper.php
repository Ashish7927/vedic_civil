    ...
    function send_email($user, $type, $shortcodes = [])
    {
        try {
            ...
            if ($status == 1) {
                ...
                if ($course_name != "") {
                    $email_subject = "You have been enrolled into:" . $course_name;
                } else {
                    $email_subject = $email_template->subj;
                }
                ...
                if ($type == "CONTACT_MESSAGE") {
                    ...
                    $to_email = "elatih@hrdcorp.gov.my";
                }
                else if($type == "Interest_form"){
                  $to_email = "nabilahashim2004@gmail.com";
                } else {
                    $to_email = $user->email;
                }

                if ($type == "course_feedback" || $type == "student_notification" || $type == "course_approve_by_reviewer") {
                    $email_subject = $email_template->subj;
                }

                if ($config->id == 1) {
                    send_php_mail($to_email, $user->name, $config->from_email, $email_subject, $message);
                } else if ($config->id == 2) {
                    send_smtp_mail($config, $to_email, $user->name, $config->from_email, Settings('site_title'), $email_subject, $message);
                } else if ($config->id == 3) {
                    sendMailBySendGrid($config, $to_email, $user->name, $config->from_email, Settings('site_title'), $email_subject, $message);
                }
                return true;
            }
            return false;
...
if (!function_exists('send_php_mail')) {
...
if (!function_exists('getProfileImage')) {
    function getProfileImage($path)
    {
        if (File::exists($path)) {
            ...
            return url('assets/profile/no_image_available.png');
        }
    }
}
...
if (!function_exists('getCourseImage')) {
    function getCourseImage($path)
    {
        if (File::exists($path)) {
            ...
            return url('assets/course/no_image.png');
        }
    }
}
...
if (!function_exists('getTestimonialImage')) {
    function getTestimonialImage($path)
    {
        if (File::exists($path)) {
            ...
            return url('demo/testimonial/thumb/img.png');
        }
    }
}
...
if (!function_exists('getInstructorImage')) {
    function getInstructorImage($path)
    {
        if (File::exists($path)) {
            ...
            return url('demo/user/instructor.jpg');
        }
    }
}
...
if (!function_exists('getStudentImage')) {
    function getStudentImage($path)
    {
        if (File::exists($path)) {
            ...
            return url('demo/user/student.png');
        }
    }
}
    ...
    function isSmeUser()
    {
        if (Auth::check()) {
            if (Auth::user()->role_id == 1) {
                ...
                if(Auth::user()->is_sme_user == 1 || \Illuminate\Support\Facades\Session::get('is_sme_user') == 1 ){
                    return true;
                } else {
                    return false;
                }
...
if (!function_exists('smeCourseSetup')) {
    function smeCourseSetup()
    {
        $setting = smeCourseSetup::first();
...
if (!function_exists('smeCourseCondition')) {
    function smeCourseCondition()
    {
        if (Auth::check()) {
            if (Auth::user()->role_id == 1) {
                return true;
                ...
                if(Auth::user()->is_sme_user == 1 || \Illuminate\Support\Facades\Session::get('is_sme_user') == 1 ){
                    \Illuminate\Support\Facades\Session::put('is_sme_user', 0);
                    $smeStatus = Session::get('sme_status');
                    if ($smeStatus != 1) {
                        return false;
                    }
                    return true;
...
if (!function_exists('isCourseReviewer')) {
    function isCourseReviewer()
    {
        if (Auth::check()) {
            if (Auth::user()->role_id == 9) {
                return true;
            } else if (Auth::user()->role->name == 'Course Reviewer') {
                return true;
            } else {
                return false;
            }
...
if (!function_exists('is_partner')) {
    function is_partner($user)
    {
        if ($user) {
            if ($user->role_id == 8) {
                return true;
            } else if (isset($user->role->name) && $user->role->name  == 'Partner') {
                return true;
            } else {
                return false;
            }
    ...
    function isPIC()
    {
        if (Auth::check()) {
            if (Auth::user()->role_id == 13) {
                return true;
            } else if (Auth::user()->role->name == 'PIC') {
                return true;
            } else {
                return false;
            }
...
if (!function_exists('isTrainer')) {
    function isTrainer($user)
    {
        if ($user) {
            if ($user->role_id == 14) {
                return true;
            } else if (isset($user->role->name) && $user->role->name  == 'Trainer') {
                return true;
            } else {
                return false;
            }
...
if (!function_exists('send_credential_info_email')) {
    function send_credential_info_email($mail_val, $type, $shortcodes = [])
    {
        try {
            $status = EmailTemplate::where('act', $type)->first()->status;
            ...
            if ($status == 1) {
                ...
                if ($config->id == 1) {
                    send_php_mail($to_email, $mail_val['send_to_name'], $config->from_email, $email_subject, $message);
                } else if ($config->id == 2) {
                    send_smtp_mail($config, $to_email, $mail_val['send_to_name'], $config->from_email, Settings('site_title'), $email_subject, $message);
                } else if ($config->id == 3) {
                    sendMailBySendGrid($config, $to_email, $mail_val['send_to_name'], $config->from_email, Settings('site_title'), $email_subject, $message);
                }
...
if (!function_exists('UserEmailNotificationSetup')) {
    function UserEmailNotificationSetup($act, $user)
    {
        ...
        /*$role_email_template = RoleEmailTemplate::where('role_id', 1)->where('template_act', $act)->where('status', 1)->first();*/
...
if (!function_exists('userRating')) {
    function userRating($user_id)
    {
        ...
        $ReviewList = DB::table('courses')
            ...
            ->where('courses.user_id', $user_id)->where('course_reveiws.status', '1')
            ->get();
...
if (!function_exists('isViewable')) {
    function isViewable($course)
    {
        ...
        if ($course->status == 0) {
            if (Auth::check()) {
                $user = Auth::user();
                if ($user->role_id != 1 && $user->role_id != 5 && $user->role_id != 6 && $course->user_id != $user->id && $user->role_id != 9 && $user->role_id != 10) {
                    $isViewable = false;
                }
            } else {
                $isViewable = false;
            ...
            if (Auth::check()) {
                // frontend view: cp and partner able to view their own course only
                if((check_whether_cp_or_not() || isPartner()) && $course->user_id != auth()->id()) {
                    $isViewable = false;
                }
            } else {
                $isViewable = true;
...
if (!function_exists('config_vimeo')) {
    function config_vimeo($random_id_length = 10)
    {
        try {
...
if (!function_exists('vimeo_folder_id_common')) {
    function vimeo_folder_id_common()
    {
        $name = (string)auth()->user()->name;
        $name = str_replace(' ', '_', $name);
        $name = strtolower($name);
        $id = auth()->user()->id . "_" . $name;
...
if (!function_exists('vimeo_upload_video_in_folder')) {
    function vimeo_upload_video_in_folder($data)
    {
        // dd($data->name);
        ...
        if (isset($data->vimeo)) {
            $file = $data->file('vimeo');
        }
        $user_id = env('VIMEO_USER_ID');
        try {
            $response = config_vimeo()->upload($file, [
                'name' => $course_name
            ]);
            ...
            $vimeo_folder_id = auth()->user()->vimeo_folder_id;

            /* Create folder */
            if (auth()->user()->vimeo_folder_id == null) {
                $id = vimeo_folder_id_common();
                $folder = config_vimeo()->request('/me/projects', [
                    'name' => $id
                ], 'POST');
                $exploded_data = explode('/', $folder['body']['uri']);
                $folder_id = $exploded_data[count($exploded_data) - 1];

                $user_data = User::find(auth()->user()->id);
                if ($user_data) {
                    $user_data->vimeo_folder_id = $folder_id;
                    $user_data->save();
                    $vimeo_folder_id = $folder_id;
                }
            }
            /* End : Create folder */
            if (isset($data->folder_id) && $data->folder_id != null) {
                $vimeo_folder_id = $data->folder_id;
            }

            /* Move Video To Folder */
            $move_url = '/users/' . $user_id . '/projects/' . $vimeo_folder_id . '/videos/' . $video_id;
            $video_move = config_vimeo()->request($move_url, [], 'PUT');
            /* End : Move Video To Folder */

            /* Folder Video List */
            $video_list_url = '/users/' . $user_id . '/projects/' . $vimeo_folder_id . '/videos';
            $video_list = config_vimeo()->request($video_list_url, [
                'per_page' => 10
            ], 'GET');
            /* End : Folder Video List */
...
if (!function_exists('get_vimeo_videos_using_folder_id')) {
    function get_vimeo_videos_using_folder_id()
    {
        ...
        $vimeo_folder_id = auth()->user()->vimeo_folder_id;
        // dd($user_id, $vimeo_folder_id);

        if ($vimeo_folder_id != null) {
            $video_list_url = '/me/projects/' . $vimeo_folder_id . '/videos';
            $data = config_vimeo()->request($video_list_url, [
                'per_page' => 10
            ], 'GET');
        }
...
if (!function_exists('assign_to_users_manager_secretary')) {
    function assign_to_users_manager_secretary($search, $video_list)
    {

        $video_list_data = $video_list;
        // dd(get_vimeo_videos_using_folder_id());
        $folder_videos = get_vimeo_videos_using_folder_id();
        // dd(isset($folder_videos['body']['error']));
        $data = [];
        if (count($folder_videos) > 0 && !isset($folder_videos['body']['error'])) {
            $video_list_data = $folder_videos['body']['data'];
        }
        if (count($video_list_data) > 0) {
            if (strlen($search) >= 3) {
                $collection_data = collect($video_list_data)->filter(function ($item) use ($search) {
                    return false !== stristr($item['name'], $search);
                });
            } else {
                $collection_data = collect($video_list_data);
            }
            $final_array = $collection_data->toArray();

            foreach ($final_array as $key => $value) {
                $data[$key]["id"] = $value["uri"];
...
if (!function_exists('random_no_for_ipay88_refno')) {
    function random_no_for_ipay88_refno()
    {
        $refno = substr(str_shuffle("0123456789abcdefghijklmnopqrstvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"), 0, 9);
        return $refno;
    }
}


if (!function_exists('signature_for_ipay88')) {
    function signature_for_ipay88($merchant_key, $merchant_code, $refno, $amount, $currency)
    {
        $amnt = str_replace('.', '', $amount);
        $amount_final = str_replace(',', '', $amnt);

        $signature = $merchant_key . $merchant_code . $refno . $amount_final . $currency;
        $hashed_signature = hash('sha256', $signature);
        return $hashed_signature;
    }
}


if (!function_exists('send_email_to_instructor_by_admin')) {
    function send_email_to_instructor_by_admin($mail_val, $type, $shortcodes = [], $type_of_mail = 0)
    {
        ...
        try {
            ...
            if ($status == 1) {
                ...
                if ($type_of_mail == 1) { //statusEnableDisable
                    $user_role = \app\User::find($mail_val['user_id'])->role_id;
                    $user = \app\User::find($mail_val['user_id']);
                    if (!empty($email_template) && ($user_role == 7 || $user_role == 2 || is_partner($user))) {
                        $message = $email_template->email_body;
                        foreach ($shortcodes as $code => $value) {
                            $message = shortcode_replacer('{{' . $code . '}}', $value, $message);
                            $course_name = "";
...
if (!function_exists('enrolled_course_add_completion_date')) {
    function enrolled_course_add_completion_date($lesson, $user_id, $course_id)
    {
        $course = \Modules\CourseSetting\Entities\Course::find($course_id);
        $progress = round($lesson->course->userTotalPercentage($user_id, $course_id));
        if ($progress) {
            if ($progress == 100) {
                $course_enrolled = \Modules\CourseSetting\Entities\CourseEnrolled::where('course_id', $course_id)->where('user_id', $user_id)->first();
                if ($course_enrolled) {
                    $course_enrolled->is_completed = 1;
                    $course_enrolled->completion_date = now();
...
if (!function_exists('check_whether_cp_or_not')) {
    function check_whether_cp_or_not()
    {
        if (auth()->user()->role->name == 'Content Provider') {
            return true;
...
if (!function_exists('html_table_for_learner_receipt')) {
    function html_table_for_learner_receipt($itemprice, $cart)
    {
        $html = '';
        $html = '<table class="custome-td-padding" style="margin: 0 auto; width: 100%;">
                    <thead><tr>
                        <th class="border_class"><b>Cource Title</b></th>
                        <th class="border_class"><b>Content Provider</b></th>
                        <th class="border_class"><b>Quantity</b></th>
                        <th class="border_class"><b>Unit Price</b></th>
                        <th class="border_class"><b>Tax</b></th>
                        <th class="border_class"><b>Amount</b></th>
                    </tr></thead>
                    <tbody>';
        // . $body['cource_details'] .
        if ($cart && $cart->count() > 0) {
            $tax = ($cart->checkout) ? $cart->checkout->total_tax : 0;
            $total_tax = $itemprice + $tax;
            $total_price = $itemprice + $total_tax;

            $html .= '
                        <tr>
                            <td class="border_class">' . $cart->course->title . '</td>
                            <td class="border_class">' . $cart->course->trainer . '</td>
                            <td class="border_class"> 1 </td>
                            <td class="border_class"> RM ' . $itemprice . '</td>
                            <td class="border_class"> RM ' . $total_tax . '</td>
                            <td class="border_class"> RM ' . $total_price . '</td>
                        <tr>
                    ';
            ...
            //     $html .= '
            //             <tr>
            //                 <td class="border_class">' . $value->course->title . '</td>
            //                 <td class="border_class">' . $value->course->trainer . '</td>
            //                 <td class="border_class"> 1 </td>
            //                 <td class="border_class"> RM ' . $itemprice . '</td>
            //                 <td class="border_class"> RM ' . $itemprice . '</td>
            //             <tr>
        ...
        $html .= '</tbody>
                    ...
                    <tr>
                        ...
                        <td class="remove-border-bottom">&nbsp;</td>
                        <td class="remove-border-bottom">&nbsp;</td>
                        <td class="border_class"><b>Total</b></td>
                        <td class="border_class">RM ' . $total_price . '</td>
                    </tr>
                    <tr>
                        <td class="remove-border-bottom">&nbsp;</td>
                        ...
                        <td class="remove-border-bottom">&nbsp;</td>
                        <td class="remove-border-bottom">&nbsp;</td>
                        <td class="border_class"><b>Paid Amount</b></td>
                        <td class="border_class">RM ' . $total_price . '</td>
                    </tr>
                    <tr>
                        <td class="remove-border-bottom">&nbsp;</td>
                        ...
                        <td class="remove-border-bottom">&nbsp;</td>
                        <td class="remove-border-bottom">&nbsp;</td>
                        <td class="border_class"><b>Balance Due</b></td>
                        <td class="border_class">RM 0</td>
                    </tr>
                    </tfoot>
                </table>';
        return $html;
...
if (!function_exists('html_table_for_cp_receipt')) {
    function html_table_for_cp_receipt($itemprice, $cart, $taxes)
    {
        $html = '';
        $quantity = 0;
        $totalAmount = $itemprice;
        $totalAmountFooter = $itemprice;
        // $html = '<table class="custome-td-padding" style="margin: 0 auto; width: 100%;">
        $html = '<table style="margin: 0 auto; width: 100%; border-collapse: collapse;">
                    <thead><tr>
                        <th style="border: 1px solid"><b>Course Title</b></th>
                        <th style="border: 1px solid"><b>Duration</b></th>
                        <th style="border: 1px solid"><b>Quantity</b></th>
                        <th style="border: 1px solid"><b>Unit Price</b></th>
                        <th style="border: 1px solid"><b>Total Amount</b></th>
                    </tr></thead>
                    <tbody>';
        // . $body['cource_details'] .
        if ($cart && $cart->count() > 0) {
            ...
            $totalAmountFooter = $subtotal;

            $html .= '
                        <tr>
                            <td style="border: 1px solid">' . $cart->course->title . '</td>
                            <td style="border: 1px solid">'.MinuteFormat($cart->course->duration).'</td>
                            <td style="border: 1px solid"> '.$quantity.' </td>
                            <td style="border: 1px solid"> RM ' . number_format($itemprice, 2, '.', '') . '</td>
                            <td style="border: 1px solid"> RM ' . number_format($subtotal, 2, '.', '') . '</td></tr>';

                            // if ($taxes) {
                            //     foreach ($taxes as $key => $tax) {
                            //         $totalAmount += ($subtotal / 100) * $tax->value;
                            //         $html .= '<td style="border: 1px solid"> RM '. number_format(($subtotal / 100) * $tax->value, 2, '.', '').'</td>';
                            //     }
                            // } 
                            
                            // $html .= '<td style="border: 1px solid"> RM '. number_format($totalAmount, 2, '.', '').'</td>
                            // </tr>';
        ...
        //             if ($taxes) {
        //                 foreach ($taxes as $key => $tax) {
        //                     $totalAmountFooter += ($subtotal / 100) * $tax->value;
        ...
        //                                 $html .= '<td class="remove-border-bottom">&nbsp;</td>';
        //                             }
        //                         $html .= '<td class="border_class"><b>'.$tax->name.'</b></td>
        //                         <td class="border_class">RM ' . ($subtotal / 100) * $tax->value . '</td>
        //                     </tr>';
        //                 }
        //             }
        ...
        //                         $html .= '<td class="remove-border-bottom">&nbsp;</td>';
        //                     }
        //                     $html .= '<td class="border_class"><b>Grand Total</b></td>
        //                     <td class="border_class">RM ' . $totalAmountFooter . '</td>
        //                 </tr>
        //             </tfoot>
        //         </table>';
        $html .= '</tbody>
            ...
            <br><br>
            <table style="margin: 0 auto; width: 20%; float: right; border-collapse: collapse;">
                <tr style="border-top: 1pt solid gray">';
                if ($taxes) {
                    foreach ($taxes as $key => $tax) {
                        $totalAmountFooter += ($subtotal / 100) * $tax->value;
                        $html .= '<td style="border: 1px solid">TOTAL SST (6%)</td>
                                <td style="border: 1px solid">RM ' . number_format(($subtotal / 100) * $tax->value, 2, '.', '') . '</td>';
                    }
                }

        $html .= '</tr>
                    <tr style="border-top: 1pt solid gray; border-bottom: 1pt solid black;">
                        <td style="border: 1px solid"><b>GRAND TOTAL</b></td>
                        <td style="border: 1px solid"><b>RM ' . number_format($totalAmountFooter, 2, '.', '') . '</b></td>
                    </tr>
                </table>
...
// if (!function_exists('html_table_for_cp_receipt')) {
//     function html_table_for_cp_receipt($itemprice, $cart)
//     {
//         $html = '';
//         $html = '<table class="custome-td-padding" style="margin: 0 auto; width: 100%;">
//                     <thead><tr>
...
//                         <td class="remove-border-bottom">&nbsp;</td>
//                         <td class="remove-border-bottom">&nbsp;</td>
//                         <td class="border_class"><b>Deduction (if applicable)</b></td>
//                         <td class="border_class">RM 0</td>
//                     </tr>
//                     <tr>
//                         <td class="remove-border-bottom">&nbsp;</td>
//                         <td class="remove-border-bottom">&nbsp;</td>
//                         <td class="border_class"><b>Net Total Pay Out</b></td>
//                         <td class="border_class">RM ' . $itemprice . '</td>
//                     </tr>
//                     <tr>
//                         <td class="remove-border-bottom">&nbsp;</td>
...
if (!function_exists('send_email_learner_receipt')) {
    function send_email_learner_receipt($mail_val, $type, $shortcodes = [])
    {
        try {
            $status = EmailTemplate::where('act', $type)->first()->status;
            if ($status == 1) {
...
if (!function_exists('rolewise_user_data_with_select2_ajax')) {
    function rolewise_user_data_with_select2_ajax($role_id)
    {
...
if (!function_exists('rolewise_user_data_with_select2_search_ajax')) {
    function rolewise_user_data_with_select2_search_ajax($is_array, $role_id, $term, $paginate)
    {
...
if (!function_exists('state_data_with_select2_search_ajax')) {
    function state_data_with_select2_search_ajax($term, $country, $paginate)
    {
        $paginate = (int)$paginate;
...
if (!function_exists('cp_data_with_select2_search_ajax')) {
    function cp_data_with_select2_search_ajax($term, $paginate)
    {
        $paginate = (int)$paginate;

        // $cpusers = User::select('id', 'name as text')
        //     ->where('role_id', 7)
        //     ->simplePaginate($paginate);

        if ($term == '') {
            // Frontend CourseListing- CP filter listed CP's name which has published their courses
            $cpusers = DB::table('users')->join('courses', 'users.id', '=', 'courses.user_id')->select('users.id', 'users.name as text')->whereIn('users.role_id', [7,8])->where('courses.status', 1)->groupBy('users.id')->simplePaginate($paginate);
        } else {
            // select2 search box by cp's name
            $cpusers = DB::table('users')->join('courses', 'users.id', '=', 'courses.user_id')->select('users.id', 'users.name as text')->whereIn('users.role_id', [7,8])->where('courses.status', 1)->where('name', 'like', '%' . $term . '%')->groupBy('users.id')->simplePaginate($paginate);
        }


        $morePages = true;
        $pagination_obj = json_encode($cpusers);
        if (empty($cpusers->nextPageUrl())) {
            $morePages = false;
        }
        $results = array(
            "results" => $cpusers->items(),
            "pagination" => array(
                "more" => $morePages
            )
        );
...
if (!function_exists('calculate_passing_marks')) {
    function calculate_passing_marks($percentage, $totalmarks)
    {
        $result = ($percentage * $totalmarks) / 100;
        return $result;
    }
}


if (!function_exists('new_course_type_users')) {
    function new_course_type_users($type_of_mail, $request, $micro_group_users, $nonmicro_group_users)
    {
        $course = \Modules\CourseSetting\Entities\Course::find($request->id);
        if ($course) {
            if ($request->course_type == 1) {
                foreach ($micro_group_users as $usergroup) {
                    $user = $usergroup->user;
                    if ($user) {
                        $mail_val = [
                        ...
                        if (check_whether_cp_or_not() || isPartner()) {
                            // send_email_to_instructor_by_admin($mail_val, 'add_new_cource', [
                            //     'company_name' => Auth::user()->getOriginal()["name"],
                            //     'course' => $request->title,
                            //     'course_type' => $request->course_type == 1 ? 'Micro-credential' : ($request->course_type == 2 ? 'Claimable course' : (($request->course_type == 3) ? 'Other' : 'E-learning')),
                            //     'course_submission_date' => date("Y-m-d H:i:s"),
                            // ], $type_of_mail);
                            send_email_to_instructor_by_admin($mail_val, 'add_new_cource', [
                                'cp_name' => Auth::user()->getOriginal()["name"],
                                'course' => $request->title,
                                'course_duration' => $request->duration,
                                'course_submission_date' => date("Y-m-d H:i:s"),
                                'course_link' => route('courseDetails', [$course->id]),
                            ], $type_of_mail);
                        } else {
                            // send_email_to_instructor_by_admin($mail_val, 'add_new_cource', [
                            //     'course' => $request->title,
                ...
                foreach ($nonmicro_group_users as $usergroup) {
                    $user = $usergroup->user;
                    if ($user) {
                        $mail_val = [
                            'send_to_name' => $user->name,
...
if (!function_exists('days_of_given_month_of_current_year')) {
    function days_of_given_month_of_current_year($month)
    {
        $d1 = date_create(date('Y') . '-' . $month); //current month/year
        if (date('F') == $month) {
            $d2 = date_create(now());
        } else {
...
if (!function_exists('enroll_day_count_calculation')) {
    function enroll_day_count_calculation($days, $enrolls)
    {
        $enroll_day = [];
        $enroll_count_arr = [];
        $enroll_count = [];
        ...
        foreach ($enrolls as $key => $enroll) {
            $enroll_count_arr[$enroll->day] = $enroll->count;
        }

        for ($i = 1; $i <= $days; $i++) {
            $enroll_day[$i] = $i;
            if (isset($enroll_count_arr[$i])) {
                $enroll_count[] = $enroll_count_arr[$i];
            } else {
                $enroll_count[] = 0;
            }
        }
        $data['enroll_day'] = $enroll_day;
        $data['enroll_count'] = $enroll_count;
...
if (!function_exists('new_course_submitted')) {
    function new_course_submitted($type_of_mail, $request, $users)
    {
        $course = \Modules\CourseSetting\Entities\Course::find($request->id);
        if ($course) {
            foreach ($users as $user) {
                ...
                if (check_whether_cp_or_not() || isPartner()) {
                    send_email_to_instructor_by_admin(
                        $mail_val,
                        'add_new_cource',
                        // [
                        //     'company_name' => Auth::user()->getOriginal()["name"],
...
if (!function_exists('course_send_email')) {
    function course_send_email($type_of_mail, $type, $request, $reviewer, $user)
    {
        $template = EmailTemplate::where('act', $type)->first();

        $course = \Modules\CourseSetting\Entities\Course::find($request->id);
        $mail_val = [
            'send_to_name' => $reviewer->name,
            'send_to' => $reviewer->email,
            'email_from' => env('MAIL_FROM_ADDRESS'),
            'email_from_name' => env('MAIL_FROM_NAME'),
...
if (!function_exists('scandir_without_dots')) {
    function scandir_without_dots($path) {
        $dirs = scandir($path);
...
if(!function_exists('get_instructor_commission')){
    function get_instructor_commission()
    {
        $commission = Settings('commission');
        $hrdc_commission = Settings('hrdc_commission');
        $instructor_commission = 100 - ($commission + $hrdc_commission);
        return $instructor_commission;
    }
}


if (!function_exists('check_if_instructor_cp_or_partner')) {
    function check_if_instructor_cp_or_partner($user_id)
    {
        $user = User::find($user_id);
        if ($user) {
            if(strtolower($user->role->name) == strtolower('Instructor')){
                return true;
            }
            else if(strtolower($user->role->name) == strtolower('Content Provider')){
                return true;
            }
            else if(strtolower($user->role->name) == strtolower('Partner')){
                return true;
            }
            else {
                return false;
...
if (!function_exists("get_instructor_total_payout")) {
    function get_instructor_total_payout($id) {
        $instructor_total = InstructorTotalPayout::where('instructor_id', $id)->first();
        if (!$instructor_total) {
            $instructor_total = new InstructorTotalPayout();
            $instructor_total->instructor_id = $id;
            $instructor_total->save();
        }
        return $instructor_total;
    }
}


if (!function_exists("get_hrdc_total_payout")) {
    function get_hrdc_total_payout($id) {
        $instructor_total = HrdcTotalPayout::where('instructor_id', $id)->first();
        if (!$instructor_total) {
            $instructor_total = new HrdcTotalPayout();
            $instructor_total->instructor_id = $id;
            $instructor_total->save();
        }
        return $instructor_total;
...
if (!function_exists("get_enroll_total")) {
    function get_enroll_total($id) {
        $enrolled_total = EnrolledTotal::where('instructor_id', $id)->first();
        if (!$enrolled_total) {
            $enrolled_total = new EnrolledTotal();
            $enrolled_total->instructor_id = $id;
            $enrolled_total->save();
        }
...
if (!function_exists('send_email_course_purchased')) {
    function send_email_course_purchased($type, $shortcodes = []) {
        try {
            $status = EmailTemplate::where('act', $type)->first()->status;
            ...
            if ($status == 1) {
                ...
                $hrdc_users = User::where('role_id', 5)->get();
                foreach($hrdc_users as $hrdc_user) {
                    $mail_val = [
                        'send_to_name' => $hrdc_user->name,
                        'send_to' => $hrdc_user->email,
                        'email_from' => env('MAIL_FROM_ADDRESS'),
                        'email_from_name' => env('MAIL_FROM_NAME'),
...
if (!function_exists("save_or_fail")) {
    function save_or_fail($record) {
        try {
            $record->save();
        } catch (\Exception $e) {
            Log::error($e->getMessage());
...
if (!function_exists('course_data_with_select2_search_ajax')) {
    function course_data_with_select2_search_ajax($term, $category_ids, $paginate)
    {
        $paginate = (int)$paginate;

        $posts = Course::withoutGlobalScope('withoutsubscription')->select('id', 'title as text')
            ->whereIn('type', [1, 2])
            ->where('status', 1)
        ...
        $posts = $posts->simplePaginate($paginate);
...
if (!function_exists('courses_data_which_not_added_with_select2_search_ajax')) {
    function courses_data_which_not_added_with_select2_search_ajax($term, $paginate, $request)
    {
        $paginate = (int)$paginate;
        $course_ids = DB::table('featured_courses')->pluck('course_id')->toArray();

        if (isset($request->type)) {
            if ($request->type == 0) { // free courses
                if ($term == "") {
                    $posts = Course::select('id', 'title as text')
                    ->where('status', 1)->where('type', 1)->where('price', 0)->where('discount_price', null)
            ...
            } else if ($request->type == 1) { // premium courses
                if ($term == "") {
                    $posts = Course::select('id', 'title as text')
...
if (!function_exists('cp_data_which_not_added_with_select2_search_ajax')) {
    function cp_data_which_not_added_with_select2_search_ajax($term, $paginate, $request)
    {
        $paginate = (int)$paginate;

        $corporate_access_page_content = app('getHomeContent');
        $cp_users = json_decode(isset($corporate_access_page_content->content_provider_list) ? $corporate_access_page_content->content_provider_list : '', true);
        $cp_user_ids = [];

        if ($cp_users) {
            foreach ($cp_users as $cp) {
                $cp_user_ids[] = $cp['id'];
            }
        }
...
if (!function_exists('corporate_data_which_not_added_with_select2_search_ajax')) {
    function corporate_data_which_not_added_with_select2_search_ajax($term, $paginate, $request)
    {
        $paginate = (int)$paginate;

        $corporate_access_page_content = app('getHomeContent');
        $c_users = json_decode(isset($corporate_access_page_content->corporate_list) ? $corporate_access_page_content->corporate_list : '', true);
        $ca_user_ids = [];

        if ($c_users) {
            foreach ($c_users as $ca) {
                $ca_user_ids[] = $ca['id'];
            }
        }
...
if (!function_exists('custom_package_for_course_data_with_select2_search_ajax')) {
    function custom_package_for_course_data_with_select2_search_ajax($term,$paginate)
    {
        $paginate = (int)$paginate;

        $posts = Course::select('id', 'title as text')
            ->whereIn('type', [1, 2])
            ->whereIn('courses.status', [1, 3]);
...
if (!function_exists('checkExternalFile')) {
    function checkExternalFile($url)
    {
        ...
        $response = ($retCode == 200) ? true : false;

        return $response;
    }
}

if (!function_exists('packages_data_which_not_added_with_select2_search_ajax')) {
    function packages_data_which_not_added_with_select2_search_ajax($term, $paginate, $request)
    {
        $paginate = (int)$paginate;
        $package_selected = DB::table('course_highlights')->pluck('package_id')->toArray();

        if ($term == "") {
            if(isPartner() || check_whether_cp_or_not()) {
                $posts = Package::select('id', 'name as text')
                ->where('status', 1)
                ->where('user_id', Auth::user()->id)
                ->whereNotIn('id', $package_selected)
                ->simplePaginate($paginate);
            } else {
                $posts = Package::select('id', 'name as text')
                ->where('status', 1)
