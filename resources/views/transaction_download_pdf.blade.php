...
<body>
    <div class="container">
        <div id="header">
            <div class="row1">
                <div class="w-70">
                    <p class="logo-content" style="font-weight: bold">{{ @$query->user->name }}</p>
                </div>

                <div class="w-30">
                    <p class="company-infor p-upper p-bold">{{ @$query->user->name }}</p>

                    @if (@$query->user->address != '' || @$query->user->address2 != '')
                        <p class="company-infor address_style">
                            {{ @$query->user->address }}, {{ @$query->user->address2 }},
                        </p>
                        <p class="company-infor address">{{ @$query->user->city_text }}</p>
                    @endif
            ...
            <div class="fax-infor">
                <table>
                    <tr>
                        ...
                        <td> TAX INVOICE : {{ @$query->invoice->getInvoice() }} </td>
                    </tr>
                    <tr>
                        <td> Wisma HRD Corp, Jalan Beringin, </td>
                        <td> Date : {{ date('d-m-Y', strtotime($query->issueDate)) }} </td>
                    </tr>
                    <tr>
            ...
            <div class="d-grid">
                <table class="list-product-table">
                    <tr>
                        ...
                        <td> QTY </td>
                        <td> UNIT PRICE </td>
                        <td> TOTAL COURSE FEE EXCLUSIVE OF 6% SST </td>
                        <td> DEDUCTION OF 30% CONSUMER FEE </td>
                        <td> TOTAL 70% OF COURSE FEE </td>
                    </tr>

                    @foreach ($datas as $data)
                        @php($grandTotal += round_to_2dp(($data['unit_price'] * $data['qty'] * (100 - Settings('commission') - Settings('hrdc_commission'))) / 100))
                        <tr>
                            <td> {{ $i++ }} </td>
                            <td> {{ $data['title'] }} </td>
                            <td> {{ $data['qty'] }} </td>
