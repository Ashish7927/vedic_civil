                                ...
                                <table id="lms_table" class="table Crm_table_active3 quiz_assign_table">
                                    ...
                                    @foreach($instance as $index => $enroll)
                                        <tr>
                                            ...
                                            <td scope="col">{{ ($enroll->activation_date) ? \Carbon\Carbon::parse($enroll->activation_date) : ''}}</td>
                                            <td scope="col">
                                                @foreach($certificateIds as $certificateId)
                                                    @if(round($enroll->course->userTotalPercentage($enroll->user_id,$enroll->course_id)) == 100 && $certificateId["course_id"] == $enroll->course->id)
                                                        <a class="dropdown-item edit_brand"
                                                           href="{{route('certificate.download', $certificateId["certificate_id"])}}">{{__('common.Download')}}</a>
                                                    @endif
                                                @endforeach
                                            </td>
                                        </tr>
