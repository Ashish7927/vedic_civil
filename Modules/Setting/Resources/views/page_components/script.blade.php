@push('scripts')
    <script type="text/javascript">
        ...
        function calCommission() {
            ...
            var hrdc_commission = document.getElementById('hrdc_commission').value;

            if (admin_comm > 100 || hrdc_commission > 100) {
                toastr.error('Commission should not more be than 100', 'Error', {
                    closeButton: true,
                    progressBar: true,
                });
            }
            if (admin_comm == "") {
                toastr.error('Please Add Admin Commission', 'Error', {
                    closeButton: true,
                    progressBar: true,
                });
            }
            if (hrdc_commission == "") {
                toastr.error('Please Add HRDC Corp Commission', 'Error', {
                    closeButton: true,
                    progressBar: true,
                });
            }
            var total_commission = parseInt(admin_comm) + parseInt(hrdc_commission);
            if(total_commission >= 100){
                toastr.error('Admin Commission + HRD Corp commission should not be 100 or more than 100', 'Error', {
                    closeButton: true,
                    progressBar: true,
                });
            }
            var result = 100 - admin_comm - hrdc_commission;
            document.getElementById('instructor_comm').value = result;

            console.log(total_commission > 100);
        }
