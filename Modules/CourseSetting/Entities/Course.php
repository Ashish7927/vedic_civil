...
//            $price= $this->offerPrice($this, $price);
    ...
    */
    ...
    public function getIsLoginUserEnrolledAttribute()
    {
        ...
        if (\auth()->user()->role_id == 1 || auth()->user()->role_id == 6 || auth()->user()->role_id == 7|| auth()->user()->role_id == 8|| auth()->user()->role_id == 9|| auth()->user()->role_id == 10) {
            return true;
        }
    ...
    public function userTotalPercentage($user_id, $course_id)
    {
        ...
        if ($countCourse != 0) {
            $percentage = ceil($countCourse / (count($this->lessons) ?: 1) * 100);
        } else {
            $percentage = 0;
        }
        if ($percentage > 100) {
...
//            $course->save();
        ...
        static::addGlobalScope('coursedata', function (Builder $builder) {
            $builder->where('corporate_id', null);
        });

        static::addGlobalScope('withoutsubscription', function (Builder $builder) {
            $builder->where('is_subscription', 0);
        });
    }
    ...
    public function courseIsQuizOrNot()
    {
        foreach($this->lessons as $lesson){
            if($lesson->is_quiz == 1 && $lesson->quiz_id != null){
                return true;
            }
        }
        return false;
    ...
    public function totalPassFailed($pass, $course_id)
    {
        $count = 0;
        $user_ids = [];
        $already_used_user_ids = [];
        foreach($this->enrollUsers as $user){
            foreach($this->lessons as $k => $lesson){
                if($lesson->is_quiz == 1 && $lesson->quiz_id != null){

                    $query = QuizTest::with('details')->where('quiz_id', $lesson->quiz_id)->where('course_id', $course_id);
                    $all = $query->where('user_id', $user->id)->latest()->first();
