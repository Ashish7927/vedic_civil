...
class FrontendManageController extends Controller
{
    ...
    public function HomeContent()
    {
        try {
            ...
            return view('frontendmanage::home_content', compact('home_content', 'pages', 'blocks', 'free_featured_courses', 'premium_featured_courses'));
        } catch (Throwable $th) {
    ...
    public function HomeContentUpdate(Request $request)
    {
        ...
        try {
            ...
            if ($request->promo_image_1 != null) {
                UpdateHomeContent('promo_image_1', $this->saveImage($request->promo_image_1));
            }

            if ($request->promo_image_2 != null) {
                UpdateHomeContent('promo_image_2', $this->saveImage($request->promo_image_2));
            }

            if ($request->promo_image_3 != null) {
                UpdateHomeContent('promo_image_3', $this->saveImage($request->promo_image_3));
            }

            if ($request->hp_advertisement_banner != null) {
                UpdateHomeContent('hp_advertisement_banner', $this->saveImage($request->hp_advertisement_banner));
            }
            ...
            $items = $request->except([
                ...
                'become_instructor_logo', 'slider_banner', 'promo_image_1',
                'promo_image_2', 'promo_image_3', 'hp_advertisement_banner', 'courses'
            ]);
            ...
            UpdateHomeContent('show_promo_section', $request->show_promo_section == 1 ? 1 : 0);
            UpdateHomeContent('show_corporate_access_promo_section', $request->show_corporate_access_promo_section == 1 ? 1 : 0);
            UpdateHomeContent('show_hp_advertisement_section', $request->show_hp_advertisement_section == 1 ? 1 : 0);
            UpdateHomeContent('show_featured_course_section', $request->show_featured_course_section == 1 ? 1 : 0);
            UpdateHomeContent('show_promo1_section', $request->show_promo1_section == 1 ? 1 : 0);
            UpdateHomeContent('show_promo2_section', $request->show_promo2_section == 1 ? 1 : 0);
            UpdateHomeContent('show_promo3_section', $request->show_promo3_section == 1 ? 1 : 0);
    ...
    public function PageContentUpdate(Request $request)
    {
        ...
        try {
            ...
            if ($request->corporate_access_page_banner != null) {
                UpdateHomeContent('corporate_access_page_banner', $this->saveImage($request->corporate_access_page_banner));
            }
            ...
            $items = $request->except([
                ...
                'course_page_banner', 'subscription_page_banner', 'corporate_access_page_banner'
            ]);
    ...
    public function testimonials_store(Request $request)
    {
        ...
        try {
            ...
            if ($request->file('image') != "") {
                ...
                $file->move('uploads/testimonial/', $image);
        ...
        try {
            ...
            if ($request->image1 != null) {
                ...
                if ($request->file('image1')->extension() == "svg") {
                    ...
                    $url1 = 'uploads/settings/' . $fileName1;
            ...
            if ($request->image2 != null) {
                ...
                if ($request->file('image2')->extension() == "svg") {
                    ...
                    $url2 = 'uploads/settings/' . $fileName2;
            ...
            if ($request->image3 != null) {
                ...
                if ($request->file('image3')->extension() == "svg") {
                    ...
                    $url3 = 'uploads/settings/' . $fileName3;
            ...
            if ($request->image4 != null) {
                ...
                if ($request->file('image4')->extension() == "svg") {
                    ...
                    $url4 = 'uploads/settings/' . $fileName4;
    ...
    private function reloadWithData()
    {
        ...
        return view('frontendmanage::sub_sec_featured_courses', compact('free_featured_courses', 'premium_featured_courses'
        ));
    }

    public function addFeaturedCourse(Request $request)
    {
        try {
                $f_course_id = $request->free_courses;
                $f_course = DB::table('featured_courses')->where('course_id', $f_course_id)->first();
                $f_num_course = DB::table('featured_courses')->where('type', 0)->count(DB::raw('DISTINCT course_id'));
                if($request->free_courses != '' && empty($f_course)) {
                    foreach ($request->free_courses as $data) {
                        $item = Course::findOrFail($data);
                        DB::table('featured_courses')
                        ->insert([
                            'type' => 0,
                            'course_id' => $data,
                            'course_title' => $item->title,
                            // 'thumbnail' => $this->saveImage($request->thumbnail),
                            'order' => $f_num_course + 1,
                            'created_at' => now(),
                            'updated_at' => now(),
                        ]);
                    }
                ...
                $p_course_id = $request->premium_courses;
                $p_course = DB::table('featured_courses')->where('course_id', $p_course_id)->first();
                $p_num_course = DB::table('featured_courses')->where('type', 1)->count(DB::raw('DISTINCT course_id'));
                if ($request->premium_courses != '' && empty($p_course)) {
                    foreach ($request->premium_courses as $data) {
                        $item = Course::findOrFail($data);
                        DB::table('featured_courses')
                        ->insert([
                            'type' => 1,
    ...
    public function editFeaturedCourseElement(Request $request)
    {
        try {
        
            $course = DB::table('featured_courses')->where('id', $request->id)->limit(1);
            if ($course) {
                if ($request->course_thumbnail != null) {
                    $image = $this->saveImage($request->course_thumbnail);
                }
                // $course->thumbnail = $request->course_thumbnail;
                $course->update(['thumbnail' => $image]);
            } else {
                Toastr::error("Can't find the course", "Failed");
                return back();
            }
            return $this->reloadWithData();
        } catch (\Exception $e) {
    ...
    public function reorderingFeaturedCourse(Request $request)
    {
        $menuItemOrder = json_decode($request->get('order'));
        $this->orderMenu($menuItemOrder);

        $featured_free_courses = DB::table('featured_courses')->where('type', 0)->orderBy('order', 'asc')->get();
        $featured_premium_courses = DB::table('featured_courses')->where('type', 1)->orderBy('order', 'asc')->get();
        UpdateHomeContent('featured_free_courses', json_encode($featured_free_courses));
        UpdateHomeContent('featured_premium_courses', json_encode($featured_premium_courses));
        return true;
    }
    ...
    private function orderMenu(array $menuItems)
    {
        ...
        foreach ($menuItems as $index => $item) {
            ...
            $menuItem = DB::table('featured_courses')->where('id', $item->id)->limit(1);
            $menuItem->update([
                'order' => $index + 1
            ]);
        }
