...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    {
        parent::boot();
        static::addGlobalScope('userdata', function (Builder $builder) {
            $builder->where('is_corporate_user', 0)->where('corporate_id', null);
        });
    }
    ...
    public function totalStudentCoursesWithData($with, $per_page, $courcefilter)
    {
        $enrolls = $this->hasMany(CourseEnrolled::class, 'user_id')->with($with)->get();
        $data_query = [];
        foreach ($enrolls as $enroll) {
            if($courcefilter == 'progress'){
                if ($enroll->course->loginUserTotalPercentage < 100) {
                    $data_query[] = $enroll;
                }
            }
            if($courcefilter == 'complete'){
                if ($enroll->course->loginUserTotalPercentage == 100) {
                    $data_query[] = $enroll;
                }
            }
        }
        $result = $this->paginate($data_query, $per_page, null, []);
        return $result;
    }
    ...
    public function trainer_cp() {
        return $this->belongsTo(User::class, 'cp_id');
    }
    
    public function company()
    {
        return $this->belongsTo(Company::class, 'corporate_id');
        // return $this->hasOne(Company::class, 'corporate_id');
    }
}
