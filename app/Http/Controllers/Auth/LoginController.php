...
//    --------------------end social lite
    ...
    public function login(Request $request)
    {
        ...
        if ($this->attemptLogin($request)) {
            ...
            if (Auth::user()->role_id == 3) {
                ...
                if (Auth::user()->is_email_changed != 1) {
                    ...
                    if (isset($apiToken) && isset($apiURL)) {
                        ...
                        if ($statusCode == 200) {
                            ...
                            if ($isSmeUser['EnrollmentID']) {
                                $student = User::where('email', $request->email)->first();
                                $student->is_sme_user = 1;
                                $student->update();
                            }
                        }
                    }
            ...
            if (isPartner()) {
                if (Auth::user()->verified_by_admin == 0) {
                    Auth::logout();
                    Toastr::error('This account is not verified by Admin yet!', 'Failed');
                    return back();
                }

                if (Auth::user()->verified_by_admin == 2) {
                    Auth::logout();
                    Toastr::error('This account has been rejected by Admin! Please contact to admin for more information', 'Failed');
                    return back();
                }
            }
            ...
            if (Auth::user()->is_email_changed == 1) {
                $data['user_data'] = $request->all();
                $data['user_data']['user_id'] = Auth::id();
    ...
    {
        return view('frontend.elatihlmstheme.auth.partner_login');
    }
    ...
    public function plogin(Request $request)
    {
        ...
        if ($this->attemptLogin($request)) {
            ...
            if (isPartner()) {
                ...
                Toastr::error('Your are not authorised !', 'Failed');
                return back();
            }
            session(['role_id' => Auth::user()->role_id]);
            if (isModuleActive('Chat')) {
...
//        $request->validate($rules);
    ...
    public function updateEmail(Request $request) {
        return view(theme('auth.email_update'));
    }

    public function updateEmailSave(Request $request) {
        $rules['email'] = "required|unique:users,email";

        $this->validate($request, $rules, validationMessage($rules));

        $session_data = $request->session()->get('user_data');
        $user = User::where('id', $session_data['user_id'])->first();

        if ($user) {
            $user->email = $request->email;
            $user->is_email_changed = 0;
            $user->save();
