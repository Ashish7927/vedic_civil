<div>
    @php
        $all_courses = [];
        $final_all_course_list = '';
        foreach($carts as $cart){
            array_push($all_courses, $cart->course->title);
        }
        if(count($all_courses)>0){
            $final_all_course_list = implode(", ", $all_courses);
        }
    @endphp
        ...
        <div class="billing_details_wrapper">
            <div class="biling_address gray-bg">
                ...
                <div class="biling_body_content">
                    ...
                    <p>{{ isset($checkout->billing->city)?$checkout->billing->cityDetails->name:'' }} - {{ @$checkout->billing->zip_code }} </p>
                                                ...
                                                <div class="deposite_payment_wrapper customer_payment_wrapper">
                                                    @if($gateway->method=="Stripe")
                                                        <form action="{{route('paymentSubmit')}}" method="post">
                                                            ...
                                                            <button type="submit" class="Payment_btn">
                                                                <img class=" w-100 " style="padding: 12px;margin-top: -9px;"src="{{asset($gateway->logo)}}"alt="">
                                                            </button>
                                                            @csrf
                                                            <script
                                                            ...
                                                            <div class="deposite_button text-center">
                                                                <form action="{{ route('paymentSubmit') }}"
                                                                    ...
                                                                    <input type="hidden" name="payment_method"
                                                                           value="{{$gateway->method}}">
                                                                    <button type="submit" class="Payment_btn">
                                                                        <img class=" w-100" style="padding: 0;margin-top: -2px;" src="{{asset($gateway->logo)}}" alt="">
                                                                    </button>
                                                                    <input type="hidden" name="tracking_id"
                                                                           value="{{$checkout->tracking}}">
                                                                    <input type="hidden" name="id"
                                                    ...
                                                    @elseif($gateway->method=="PayTM")
                                                        ...
                                                        <form action="{{route('paymentSubmit')}}" method="post">
                                                            ...
                                                            <input type="hidden" name="id" value="{{$checkout->id}}">
                                                            <button type="submit" class="Payment_btn">
                                                                <img class=" w-100" style="    padding: 10px;margin-top: -6px;" src="{{asset($gateway->logo)}}" alt="">
                                                            </button>
                                                    ...
                                                    @elseif($gateway->method=="Ipay88")
                                                        <form action="{{route('paymentSubmit')}}" method="post">
                                                            @csrf
                                                            <input type="hidden" name="payment_method" value="Ipay88">
                                                            <input type="hidden" name="tracking_id" value="{{$checkout->tracking}}">
                                                            <input type="hidden" name="product_des" value="{{$final_all_course_list}}">
                                                            <input type="hidden" name="id" value="{{$checkout->id}}">
                                                            <button type="submit" class="Payment_btn">
                                                                <img class=" w-100" style="padding: 10px;margin-top: -6px;" src="{{asset('public/demo/gateway/ipay88.png')}}" alt="">
                                                            </button>
                                                        </form>
                                                    @else
        ...
        <div class="order_wrapper">
            ...
            <div class="ordered_products_lists">
                ...
                @if($checkout->purchase_price > 0)
                    <div class="single_lists" style="display: {{$checkout->discount !=0?'block':'none'}}">
                    ...
                    @php $taxes = json_decode($checkout->tax_json); @endphp
                    @if ($taxes)
                        @foreach ($taxes as $tax)
                            <div class="single_lists">
                                <span class="total_text">{{ $tax->name }} </span>
                                <span class="totalTax">{{ getPriceFormat($tax->tax) }}</span>
                            </div>
                        @endforeach
                    @endif
