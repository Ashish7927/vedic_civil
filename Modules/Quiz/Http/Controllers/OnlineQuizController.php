...
class OnlineQuizController extends Controller
{
    ...
    public function CourseQuizStore(Request $request)
    {
        ...
        if ($request->type == 2) {
            ...
            try {
                ...
                $online_exam->quiz_feedback = $request->quiz_feedback;
                $online_exam->created_by = Auth::user()->id;
                $result = $online_exam->save();

                $user = Auth::user();
                if ($user->role_id == 1 || $user->role_id == 5 || $user->role_id == 6) {
                    $course = Course::withoutGlobalScope('withoutsubscription')->where('id', $request->course_id)->first();
                } else {
                    $course = Course::withoutGlobalScope('withoutsubscription')->where('id', $request->course_id)->where('user_id', Auth::id())->first();
                }
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            ...
            'percentage' => 'required',
            'instruction' => 'required',
            'question_time' => 'min:0|not_in:0'
        ];

        $messages = [
            'question_time.min' => 'Minimum time should be 0.',
            'question_time.not_in' => 'Question time should be more than 0.',
        ];
        ...
        try {
            ...
            $online_exam->is_enabled = $request->is_enabled;
            $online_exam->enable_quiz_feedback = $request->enable_quiz_feedback;

            $online_exam->pass_message = $request->pass_message;
            $online_exam->fail_message = $request->fail_message;
            if (isset($request->use_default_messages)) {
                $online_exam->display_result_message = $request->use_default_messages;
            } else {
                $online_exam->display_result_message = 0;
            }
    ...
    public function onlineExamQuestionAssignByAjax(Request $request)
    {
        ...
        try {
            ...
            if (isset($request->questions)) {
                ...
                $passing_marks = calculate_passing_marks($online_exam->percentage, $online_exam->total_marks);
