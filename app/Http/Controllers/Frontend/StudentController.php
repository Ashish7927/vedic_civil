...
class StudentController extends Controller
{
    ...
    public function learnerActivationCourse(Request $request)
    {
        try {
            $courseEnrolled = CourseEnrolled::where('id', $request->id)->first();

            $courseEnrolled->status = 1;
            $courseEnrolled->activation_date = date("Y-m-d H:i:s");
            $courseEnrolled->save();

            Toastr::success(trans('common.Operation successful'), trans('common.Success'));
            return redirect()->to(route('courseDetailsView',$courseEnrolled->course->slug));
        } catch (\Exception $e) {
    ...
    public function myCertificate(Request $request)
    {
        ...
        try {
            return view(theme('pages.myCertificate'), compact('request'));
        } catch (\Exception $e) {
            GettingError($e->getMessage(), url()->current(), request()->ip(), request()->userAgent());
        }
    }
    public function myHistory()
    {
        ...
        try {
            return view(theme('pages.myHistory'));
        } catch (\Exception $e) {
            GettingError($e->getMessage(), url()->current(), request()->ip(), request()->userAgent());
        }
    }
...
// dd($request) ;
    ...
    public function myProfileUpdate(Request $request)
    {
        ...
        if (Auth::user()->role_id == 1) {
            ...
            $validate_rules = [
                ...
                'phone' => 'nullable|string|regex:/^([0-9\s\-\+\(\)]*)$/|min:1,phone,' . Auth::id(),
        ...
        try {
            ...
            $user->not_working = $request->not_working;
    ...
    public function MyUpdatePassword(Request $request)
    {
        $this->validate($request, [
            ...
            'new_password' => ['required', 'min:8', 'regex:/[a-z]/', 'regex:/[A-Z]/', 'regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*(_|[^\w])).+$/'],
            'confirm_password' => 'required_with:new_password|same:new_password|min:8'
        ],[
         'new_password.required' => 'Please enter password',
         'new_password.same' => 'Password not matched',
         'new_password.confirmed' => 'Password not matched',
         'new_password.regex' => 'minimum 8 charaters including 1 capital letters and 1 unique symbol',
         'new_password.string' => 'Please insert minimum 8 charaters',
         'max' => 'The :attribute field is may not be greater than :max.',
         'min' => 'The :attribute field is may not be less than :min.' ]);
        try {
            if (demoCheck()) {
                return redirect()->back();
            }
...
//                        $couponApply=true;
    ...
    public function getCertificate($id, $slug, Request $request)
    {
        ...
        try {
            ...
            $title = $certificate_record->certificate_id;
    ...
    public function submitReview(Request $request)
    {
        ...
        try {
            ...
            if (is_null($review)) {
                ...
                $course_user = User::findOrFail($user_id);
    ...
    {
        $courses_enroll = CourseEnrolled::with('course', 'course.user');
        $courses_enroll->where("user_id", Auth::user()->id)->where('is_completed',1);

        $courses_enroll = $courses_enroll->get();
        $data = [
        ...
        //return view('learnerTranscript', $data);
        $pdf = PDF::loadView('learnerTranscript', $data);

        return $pdf->download('LearningTranscript.pdf');
    }
