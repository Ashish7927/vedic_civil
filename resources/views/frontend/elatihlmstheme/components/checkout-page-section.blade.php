            ...
            <div class="billing_details_wrapper">
                ...
                <div class="d-none d-sm-none d-md-block d-lg-block d-xl-block">
                    ...
                    <table class="table table-bordered billing_info"
                        ...
                        <tr>
                            ...
                            <td class="billing_city">{{isset($bills[0]->city_text)?$bills[0]->city_text:''}}</td>
                        </tr>
                        <tr>
                            <td>{{__('State')}}</td>
                ...
                <div class="row billing_form" style=" @if(count($bills) > 0) display: none @endif">
                    ...
                    <div class="col-lg-12 mt_20">
                        ...
                        <select class=" select2 wide cityList" name="city" id="city">
                            <option
                                ...
                                value="">{{__('common.Select')}} {{__('State')}}
                            </option>
                            @foreach ($cities as $city)
                                {{-- <option value="{{@$city->id}}" @if ($current->city==$city->id) selected @endif>{{@$city->name}}</option> --}}
                    ...
                    <div class="col-lg-12 mt_20">
                        ...
                        <input id="city_text" name="city_text" placeholder="{{__('Enter City')}}"
                               onfocus="this.placeholder = ''" class="primary_input3"
                               type="text"
                               value="@if(!empty($current)){{$current->city_text}}@else{{old('city_text')}}@endif">
                    </div>
                    <div class="col-lg-12 mt_20">
                        ...
                        <select id="country" name="country"
                            ...
                            @if(isset($countries))
                                @foreach($countries as $country)
                                    @php
                                        $selected = '';
                                        if(!empty($current) && $current->country==$country->id){
                                            $selected = 'selected';
                                        }
                                        elseif(!empty($profile) && $profile->country==$country->id){
                                            $selected = 'selected';
                                        }
                                        elseif($country->id == 132 || $country->name == 'Malaysia'){
                                            $selected = 'selected';
                                        }
                                    @endphp
            ...
            <div class="order_wrapper">
                        ...
                        @foreach($carts as $cart)
                            ...
                            <div class="single_ordered_product">
                                <div class="product_name d-flex align-items-center">
                                    <div class="thumb" style="background-image: url('{{getCourseImage(@$cart->course->thumbnail)}}')"></div>
                ...
                <div class="ordered_products_lists">
                    ...
                    @if($checkout->purchase_price > 0)
                        ...
                        <div class="single_lists" id="discountBox" style="display: {{ $checkout->discount != 0 ? 'block' : 'none' }}">
                        ...
                        @if ($taxes)
                          @foreach ($taxes as $tax)
                              @php
                                  $taxValue = ($checkout->price / 100) * $tax->value;
                                  $taxAmt += $taxValue;
                              @endphp

                              <div class="single_lists">
                                  <span class="total_text">{{ $tax->name }} </span>
                                  <span class="totalTax">{{ getPriceFormat($taxValue) }}</span>
                              </div>
                          @endforeach
                        @endif

                        @php
                            $payableAmt = $checkout->purchase_price + $taxAmt;
                        @endphp
                        <div class="single_lists">
                            <input type="hidden" name="payable_amt" value="{{ number_format($payableAmt, 2) }}">
                            <span class="total_text">{{ __('frontend.Payable Amount') }} </span>
                            @if(hasTax())
                                <span class="totalBalance">{{ getPriceFormat($payableAmt) }}</span>
                            @else
                                <span class="totalBalance">{{ getPriceFormat($payableAmt) }}</span>
                            @endif
                ...
                <div class="bank_transfer">
                    {{-- <p class="mb_35">{{__("frontend.Your personal data will be used to process your order, support your experience throughout this website, and for other purposes described in our privacy policy")}}
                        .</p> --}}
                        {{-- <label class="primary_checkbox d-flex primary_checkbox_for_checkout" for="checkbox">
                            <input name="checkbox" type="checkbox" id="checkbox">
                            <span class="checkmark mr_15"></span>
                            <p class="mb_35">I hereby agrees that the total amount of this order to be charged by My Live Learning Sdn Bhd (1436583M) with address registered at H-20-01, Block H, Empire City, No.8, Jalan Damansara PJU 8, Damansara Perdana, 47820 Petaling Jaya, Selangor, Malaysia.</p>
                        </label> --}}
                        <label class="primary_checkbox d-flex primary_checkbox_for_checkout" for="checkbox_1">
                            <input name="checkbox_1" type="checkbox" id="checkbox_1">
                            <span class="checkmark mr_15" style="margin-top: 50px"></span>
                            <input type="hidden" name="refund_policy_url" value="" id="refund_policy_url"/>
                            {{-- <p class="mb_35">I hereby agree to the e-LATiH Premium <a href="#" target="_blank" id="click-checkout-refund-policy">Refund Policy</a> and I will activate the purchased course within 48 hours after the payment is successful.</p> --}}
                            <p class="mb_35">I have read and agree to the <a href="{{ asset('pages/terms-of-use') }}" target="_blank">e-LATiH Terms of Use</a> and <a href="{{ asset('pages/refund-policy-1') }}" target="_blank">Refund Policy</a>. I understand that I have to activate the purchased course within 48 hours after the payment is successful.</p>
                        </label>
                    <button type="submit" id="submitBtn" style="text-transform: none;"
                            class="theme_btn w-100"><b>{{__('frontend.Place An Order')}}</b></button>
                </div>
            </div>
        </div>
    </form>
...
<script>
    var session = <?= \Illuminate\Support\Facades\Session::get('checkbox') ?>;
    var session_1 = <?= \Illuminate\Support\Facades\Session::get('checkbox_1') ?>;
    if (session === 1) {
        $('#checkbox').focus();
    }
    $('#click-checkout-refund-policy').click(function () {
        $('#refund_policy_url').val(1);
    });
</script>
