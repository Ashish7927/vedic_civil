...
class CreateBbbSettingsTable extends Migration
{
    public function up()
    {
        Schema::create('bbb_settings', function (Blueprint $table) {
            ...
            $table->set('guest_policy',['ALWAYS_ACCEPT','ALWAYS_DENY','ASK_MODERATOR'])->default('ALWAYS_ACCEPT');
            $table->boolean('redirect')->default(true);
            $table->boolean('join_via_html_5')->default(true);
            $table->set('state',['any','published','unpublished'])->default('any');

            $table->timestamps();
        });

        \Illuminate\Support\Facades\DB::table('bbb_settings')->insert([
            'password_length' => '8'
        ]);
    }
